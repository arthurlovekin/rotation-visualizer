// ---------------------------------------------------------------------------
// Private helpers — shared between VectorFormat and MatrixFormat
// ---------------------------------------------------------------------------
...
fn detect_delimiter_and_parse(inner: &str) -> Result<(String, Vec<f64>), String> {
    ...
    if trimmed.contains(',') {
        ...
        let numbers = split_and_parse(trimmed, ",")?;
    ...
    let numbers: Result<Vec<f64>, String> =
        parts.iter().map(|s| parse_single_number(s)).collect();
...
fn split_and_parse(text: &str, delimiter: &str) -> Result<Vec<f64>, String> {
    text.split(delimiter)
        .map(|s| parse_single_number(s.trim()))
        .collect()
}
...
///
/// Returns `(open_vector, close_vector, row_delimiter, row_contents)`.
///
...
fn detect_and_split_rows(
    ...
    if let Some((open, close)) = bracket_pair {
        let (row_contents, row_delimiter) =
            split_nested_rows(trimmed, open, close)?;
    ...
    if trimmed.contains(';') {
        let row_delimiter = if trimmed.contains("; ") { "; " } else { ";" };
...
/// and detect the row delimiter from the text between closing and opening brackets.
///
...
fn split_nested_rows(
    ...
    let mut row_delimiter: Option<String> = None;
    let mut last_close_end: Option<usize> = None;

    for (i, ch) in inner.char_indices() {
        if ch == open {
            if depth == 0 {
                // Capture row_delimiter from text between previous row's close and this open
                if row_delimiter.is_none() {
                    if let Some(lce) = last_close_end {
                        row_delimiter = Some(inner[lce..i].to_string());
                    }
                }
    ...
    let delim = row_delimiter.unwrap_or_else(|| ", ".to_string());
    Ok((rows, delim))
}
...
impl VectorFormat {
    ...
    ///
    /// `expected_len` is the exact number of values expected in the vector.
    /// Returns an error if the input is malformed or contains a different
    ...
    pub fn detect_and_parse(
        input: &str,
        expected_len: usize,
    ) -> Result<(VectorFormat, Vec<f64>), String> {
        ...
        let content = strip_wrappers(trimmed);

        // Validate bracket balance before parsing
        validate_brackets(content)?;

        // Detect and strip brackets
        let (open_bracket, close_bracket, inner) = detect_brackets(content);

        // Detect delimiter and parse numbers from inner content
        let (number_delimiter, numbers) = detect_delimiter_and_parse(inner)?;
...
/// Represents the text format detected from user input for a matrix (2D array).
/// Stores the bracket and delimiter scheme used for representing matrices as text.
#[derive(Debug, Clone, PartialEq)]
pub struct MatrixFormat {
    /// Format for individual row vectors (brackets + number delimiter)
    pub row_format: VectorFormat,
    /// Delimiter between row vectors, e.g. ", " or "; " or ","
...
impl Default for MatrixFormat {
    fn default() -> Self {
        Self {
            row_format: VectorFormat::default(),
            row_delimiter: ", ".to_string(),
            open_bracket: "[".to_string(),
            close_bracket: "]".to_string(),
        }
...
impl MatrixFormat {
    ...
    pub fn detect_and_parse(input: &str) -> Result<(MatrixFormat, Vec<Vec<f64>>), String> {
        let trimmed = input.trim();
        if trimmed.is_empty() {
            return Err("Empty input".to_string());
        }
        ...
        let (open_vector, close_vector, row_delimiter, row_contents) =
            detect_and_split_rows(inner)?;
        ...
        for row_str in &row_contents {
            ...
            let (delim, nums) = detect_delimiter_and_parse(trimmed)?;
        ...
        let format = MatrixFormat {
            row_format: VectorFormat {
                number_delimiter: number_delimiter.unwrap_or_else(|| ", ".to_string()),
                open_bracket: open_vector,
                close_bracket: close_vector,
            },
            row_delimiter,
            open_bracket,
            close_bracket,
    ...
    /// brackets and delimiters.
    ///
    /// Each row is formatted as a vector (using the inner `row_format`), then
    /// rows are joined by `row_delimiter` and wrapped in matrix brackets.
    /// This handles all styles uniformly:
    ...
    pub fn format_matrix(&self, rows: &[Vec<f64>]) -> String {
        let row_strs: Vec<String> = rows
            .iter()
            .map(|row| self.row_format.format_vector(row))
            .collect();
        format!(
            "{}{}{}",
            self.open_bracket,
            row_strs.join(&self.row_delimiter),
            self.close_bracket
        )
    }
}
...
///
/// Accepts any format that [`VectorFormat::detect_and_parse`] understands.
...
) -> Result<(VectorFormat, Rotation), String> {
    let (fmt, nums) = VectorFormat::detect_and_parse(input, 4)?;
...
///
/// Returns `(detected_format, rotation)` on success.
pub fn parse_axis_angle_3d_str(input: &str) -> Result<(VectorFormat, Rotation), String> {
    let (fmt, nums) = VectorFormat::detect_and_parse(input, 3)?;
...
mod tests {
    ...
    fn detect(input: &str, n: usize) -> (VectorFormat, Vec<f64>) {
        VectorFormat::detect_and_parse(input, n).unwrap()
    }

    fn detect_matrix(input: &str) -> (MatrixFormat, Vec<Vec<f64>>) {
        MatrixFormat::detect_and_parse(input).unwrap()
    }
    ...
    fn square_bracket_comma_space() {
        let (fmt, nums) = detect("[1.0, 2.0, 3.0, 4.0]", 4);
        assert_eq!(fmt.open_bracket, "[");
        assert_eq!(fmt.close_bracket, "]");
    ...
    fn paren_comma_space() {
        let (fmt, nums) = detect("(0.0, 0.0, 0.0, 1.0)", 4);
        assert_eq!(fmt.open_bracket, "(");
        assert_eq!(fmt.close_bracket, ")");
        assert_eq!(fmt.number_delimiter, ", ");
    ...
    fn bare_comma_separated() {
        let (fmt, nums) = detect("1.0, 2.0, 3.0", 3);
        assert_eq!(fmt.open_bracket, "");
        assert_eq!(fmt.close_bracket, "");
        assert_eq!(fmt.number_delimiter, ", ");
    ...
    fn numpy_wrapper_stripped() {
        let (fmt, nums) = detect("np.array([0.0, 0.0, 0.0, 1.0])", 4);
        assert_eq!(fmt.open_bracket, "[");
        assert_eq!(fmt.close_bracket, "]");
        assert_eq!(fmt.number_delimiter, ", ");
        assert_eq!(nums, vec![0.0, 0.0, 0.0, 1.0]);
    ...
    fn rust_vec_macro_stripped() {
        let (fmt, nums) = detect("vec![1.0, 2.0, 3.0]", 3);
        assert_eq!(fmt.open_bracket, "[");
        assert_eq!(fmt.close_bracket, "]");
        assert_eq!(nums, vec![1.0, 2.0, 3.0]);
    }
    ...
    fn rust_f32_suffixes_parsed() {
        let (_, nums) = detect("[1.0f32, 2.5f64, 3.0]", 3);
        assert_eq!(nums, vec![1.0, 2.5, 3.0]);
    }

    #[test]
    fn rust_underscore_separators() {
        let (_, nums) = detect("[1_000.0, 2_000.0]", 2);
        assert_eq!(nums, vec![1000.0, 2000.0]);
    }

    #[test]
    fn negative_numbers() {
        let (_, nums) = detect("[-1.0, 2.0, -3.5]", 3);
    ...
    fn empty_input_is_err() {
        assert!(VectorFormat::detect_and_parse("", 4).is_err());
        assert!(VectorFormat::detect_and_parse("   ", 4).is_err());
    }
    ...
    fn wrong_count_is_err() {
        // Expect 4 but get 3
        assert!(VectorFormat::detect_and_parse("[1.0, 2.0, 3.0]", 4).is_err());
        // Expect 3 but get 4
        assert!(VectorFormat::detect_and_parse("[1.0, 2.0, 3.0, 4.0]", 3).is_err());
    }

    // ===================================================================
    // 2. MatrixFormat detection — matrix inputs
    // ===================================================================
    ...
    fn matrix_nested_square_brackets() {
        ...
        assert_eq!(fmt.open_bracket, "[");
        assert_eq!(fmt.close_bracket, "]");
        assert_eq!(fmt.row_format.open_bracket, "[");
        assert_eq!(fmt.row_format.close_bracket, "]");
        assert_eq!(fmt.row_format.number_delimiter, ", ");
    ...
    fn matrix_matlab_semicolons() {
        ...
        assert_eq!(fmt.open_bracket, "[");
        assert_eq!(fmt.close_bracket, "]");
        // Matlab-style: row brackets are empty, rows separated by "; "
        assert_eq!(fmt.row_format.open_bracket, "");
        assert_eq!(fmt.row_format.close_bracket, "");
        assert_eq!(fmt.row_delimiter, "; ");
        assert_eq!(rows.len(), 3);
    ...
    fn matrix_empty_is_err() {
        assert!(MatrixFormat::detect_and_parse("").is_err());
        assert!(MatrixFormat::detect_and_parse("[]").is_err());
    }
    ...
    fn round_trip_vector() {
        ...
        let (fmt, nums) = detect(input, 4);
        let formatted = fmt.format_vector(&nums);
        let (_, nums2) = detect(&formatted, 4);
    ...
    fn uneven_spaces_around_commas() {
        ...
        let (_, nums) = detect("[1.0,  2.0,   3.0,4.0]", 4);
        assert_eq!(nums, vec![1.0, 2.0, 3.0, 4.0]);
    }
    ...
    fn tabs_and_spaces_mixed() {
        let (_, nums) = detect("1.0\t 2.0 \t3.0", 3);
        assert_eq!(nums, vec![1.0, 2.0, 3.0]);
    }
    ...
    fn numpy_wrapper_inner_whitespace() {
        ...
        let (_, nums) = detect("np.array([ 1.0,  2.0,   3.0 ])", 3);
        assert_eq!(nums, vec![1.0, 2.0, 3.0]);
    }
    ...
    fn rust_vec_macro_inner_whitespace() {
        let (_, nums) = detect("vec![ 1.0,  2.0,   3.0 ]", 3);
        assert_eq!(nums, vec![1.0, 2.0, 3.0]);
    }
    ...
    fn malformed_unclosed_square_bracket() {
        let err = VectorFormat::detect_and_parse("[1.0, 2.0, 3.0", 3).unwrap_err();
    ...
    fn malformed_unclosed_curly() {
        let err = VectorFormat::detect_and_parse("{1.0, 2.0", 2).unwrap_err();
        assert!(err.contains("Unclosed"), "expected unclosed error, got: {}", err);
    }
    ...
    fn malformed_numpy_wrapper_bad_inner() {
        ...
        let err = VectorFormat::detect_and_parse("np.array([1.0, 2.0, 3.0)", 3).unwrap_err();
    ...
    fn malformed_matrix_unclosed_outer() {
        let err = MatrixFormat::detect_and_parse("[[1, 2], [3, 4]").unwrap_err();
        assert!(err.contains("Unclosed"), "expected unclosed error, got: {}", err);
    }
    ...
    fn inconsistent_delimiters_comma_and_semicolon() {
        ...
        assert!(VectorFormat::detect_and_parse("[1, 2; 3, 4]", 4).is_err());
    }
